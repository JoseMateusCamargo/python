...
def map(data, lat, lon, zoom):
    st.write(pdk.Deck(
        ...
        layers=[
            pdk.Layer(
                "HexagonLayer", data=data, get_position=["lon", "lat"], radius=100, elevation_scale=4,
...
filtered = data[(data[DATE_TIME].dt.hour >= hour_selected) & (data[DATE_TIME].dt.hour < (hour_selected + 1))]
...
st.altair_chart(alt.Chart(chart_data).mark_area(interpolate='step-after', ).encode(
    x=alt.X("minute:Q", scale=alt.Scale(nice=False)), y=alt.Y("pickups:Q"),
    tooltip=['minute', 'pickups']).configure_mark(opacity=0.2, color='red'), use_container_width=True)
